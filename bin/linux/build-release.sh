...
mkdir -p Release
cd Release
RELEASE_APP_DIR="${ROOT_DIR}/Release"
...
#cd "${RELEASE_APP_DIR}"/_build/prod/rel/*/lib/crypto-*/priv/lib

## Use otool to list linked libraries and grep for OpenSSL, then extract the first path
#openssl_lib=$(ldd crypto.so | grep libcrypto | awk '{print $3}')
#
